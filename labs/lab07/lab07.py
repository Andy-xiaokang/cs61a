...
def inc_subseqs(s):
    ...
    def subseq_helper(s, prev):
        if not s:
            ...
            a = subseq_helper(s[1:], s[0]) # inclue s[0] inc_subseq
            b = subseq_helper(s[1:], prev) # exclue s[0] inc_subseq
...
def make_glookup(class_assignments):
    ...
    total_point = 0
    total_get = 0
    def grade_percentage(assignment, point):
        nonlocal total_point, total_get
        total_point += class_assignments[assignment]
        total_get += point
        return total_get/total_point
...
def make_advanced_counter_maker():
    ...
    def make_counter():
        counter = 0
        def tom_counter(message):
            nonlocal global_counter, counter
            match message:
                case "count":
                    counter += 1
                    return counter
