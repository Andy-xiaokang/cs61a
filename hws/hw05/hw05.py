...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, input_password):
        nonlocal balance, error_password
        if len(error_password) == 3:
            return f"Too many incorrect attempts. Attempts: {error_password}"
        if input_password != password:
            error_password.append(input_password)
