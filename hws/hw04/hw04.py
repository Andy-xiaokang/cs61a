...
def preorder(t):
    ...
    for branch in branches(t):
        preorder_list = preorder_list + preorder(branch)
...
def quadratic(x, a, b, c):
    ...
    """
    "*** YOUR CODE HERE ***"
    x1, x2, x0 = lower_bound(x), upper_bound(x), -b/(2*a)
    f1, f2, f0 = a*x1**2 + b*x1 + c, a*x2**2 + b*x2 + c, a*x0**2 + b*x0 + c
    if x1 <= x0 <= x2:
        return interval(min([f1, f2, f0]), max(f1, f2, f0))
    else:
        return interval(min([f1, f2]), max([f1, f2]))

# r1 and r2 is a interval, use such computation is wrong
def par1(r1, r2):
