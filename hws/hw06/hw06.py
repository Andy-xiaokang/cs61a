...
def is_bst(t):
    ...
    if t.is_leaf():
        ...
        if left.label <= t.label:
            return is_bst(left) and bst_max(left) <= t.label
        else:
            return is_bst(right) and bst_min(right) > t.label
    elif len(t.branches) == 2:
        left, right = t.branches[0], t.branches[1]
        if not bst_max(left) <= t.label < bst_min(right):
            return False
        flag = True
        return flag and is_bst(left) and is_bst(right)
...
def remove_all(link , value):
    ...
    """
    "*** YOUR CODE HERE ***"
    p, current = link, link.rest
    while current != Link.empty:
        if current.first == value:
            p.rest = current.rest
            current = current.rest
        else:
            p, current = current, current.rest
