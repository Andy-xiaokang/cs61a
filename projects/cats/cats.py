...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    def contain_topic_select(paragraph):
        words = split(lower(remove_punctuation(paragraph)))
        for i in topic:
            if i in words:
                return True
        return False
    return contain_topic_select
    # END PROBLEM 2
...
def accuracy(typed, reference):
    ...
    return round(100*correct_words_number/len(typed_words), 2)
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    words_number = len(typed)/5
    return round(60 * words_number/elapsed, 2)
    # END PROBLEM 4
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if user_word in valid_words:
        return user_word
    diff_value_list = [diff_function(user_word, valid_word, limit) for valid_word in valid_words]
    min_diff_value = min(diff_value_list)
    min_diff_index = diff_value_list.index(min_diff_value)
...
def fastest_words(game):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    fastest_player_words_list = []
    for i in players:
        fastest_player_words_list.append([]) # initialize list
    for i in words:
        smallest_time = time(game, 0, i)
        for j in players:
            if time(game, j, i) < smallest_time:
                smallest_time = time(game, j, i)
        for j in players:
            if time(game, j,i) == smallest_time:
                fastest_player_words_list[j].append(word_at(game, i))
                break         
    return fastest_player_words_list
    # END PROBLEM 10
...
memo_for_fa = {}
# can't figure out, reference from flying pig but can't pass test
