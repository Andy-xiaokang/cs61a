...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        current_index_to_self = 0
        while current_index_to_self < self.min_range and current_place is not beehive:
            current_place = current_place.entrance
            current_index_to_self += 1
        while  current_index_to_self <= self.max_range and  current_place is not beehive:
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        # if bee's direction is -1
            #if the bee is already right next to the hive
                #don't move
            #otherwise
                #change destination to one space toward the Hive
        # END EC
        if self.blocked():
...
def make_slow(action, bee):
    ...
    def slow_action(gamestate):
        if gamestate.time % 2 == 0:
            action(gamestate)
        #if gamestate.time is even,then
            #perform the bee's original function
        #else
            #do nothing
    return slow_action
    # END Problem EC
...
def make_scare(action, bee):
    ...
    def scare_action(gamestate):
        ...
        #perform the bee's action
        #change the bee's directon to +1
    return scare_action
    # END Problem EC
...
def apply_status(status, bee, length):
    ...
    # original_action = bee's origianl action
    # status_action = bee's new modified action
    # somewhere we want to modify the bee's action
    # END Problem EC
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        if target and target.scared == False:
            ...
            target.scared = True
        # if target is present and the target has not been scared before
            # call apply_status with the correct parameters
            # note that the target has been scared
        # END Problem EC
...
class LaserAnt(ThrowerAnt):
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem OPTIONAL
        eval_damage = self.damage - 0.2*distance - 0.05*self.insects_shot
        return eval_damage if eval_damage > 0 else 0
        # END Problem OPTIONAL
